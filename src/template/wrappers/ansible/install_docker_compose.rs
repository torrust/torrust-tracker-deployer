...
pub struct InstallDockerComposeTemplate {
    #[allow(dead_code)]
    context: InstallDockerComposeContext,
    content: String,
}

#[derive(Serialize, Debug)]
struct InstallDockerComposeContext {
    // No template variables for now - this is a static template
}

impl TemplateContext for InstallDockerComposeContext {
    // No required methods - Tera handles validation
}

impl InstallDockerComposeTemplate {
    /// Creates a new `InstallDockerComposeTemplate`, validating the template content and variable substitution
    ///
    /// # Errors
    /// 
    /// Returns an error if:
    ...
    pub fn new(template_content: &str) -> Result<Self> {
        ...
        let context = InstallDockerComposeContext {};

        // Create a temporary engine with the template content
        let template_name = "install-docker-compose.yml";
        let engine =
...
impl TemplateRenderer for InstallDockerComposeTemplate {
    ...
    fn template_path(&self) -> &Path {
        ...
        Path::new("install-docker-compose.yml")
    }
