...
pub struct WaitCloudInitTemplate {
    #[allow(dead_code)]
    context: WaitCloudInitContext,
    content: String,
}

#[derive(Serialize, Debug)]
struct WaitCloudInitContext {
    // No template variables for now - this is a static template
}

impl TemplateContext for WaitCloudInitContext {
...
impl WaitCloudInitTemplate {
    /// Creates a new `WaitCloudInitTemplate`, validating the template content and variable substitution
    ///
    /// # Errors
    /// Returns an error if:
    ...
    pub fn new(template_content: &str) -> Result<Self> {
        ...
        let context = WaitCloudInitContext {};

        // Create a temporary engine with the template content
        let template_name = "wait-cloud-init.yml";
        let engine =
