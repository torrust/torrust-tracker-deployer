...
fn it_should_create_environment_from_valid_config() {
    ...
    let result = create::handle(&config_path, temp_dir.path());
...
fn it_should_reject_nonexistent_config_file() {
    ...
    let result = create::handle(&nonexistent_path, temp_dir.path());
...
fn it_should_reject_duplicate_environment() {
    ...
    let result1 = create::handle(&config_path, temp_dir.path());
    assert!(result1.is_ok(), "First create should succeed");

    // Try to create same environment again
    let result2 = create::handle(&config_path, temp_dir.path());
...
fn it_should_create_environment_in_custom_working_dir() {
    ...
    let result = create::handle(&config_path, &custom_working_dir);
...
fn it_should_provide_help_for_all_error_types() {
    ...
    if let Err(e) = create::handle(&nonexistent, temp_dir.path()) {
        let help = e.help();
    ...
    if let Err(e) = create::handle(&invalid_json, temp_dir.path()) {
        let help = e.help();
        assert!(!help.is_empty());
    ...
    if let Err(e) = create::handle(&invalid_name, temp_dir.path()) {
        let help = e.help();
        assert!(!help.is_empty());
