...
pub(in crate::presentation::cli::views) struct StdoutWriter(Box<dyn Write + Send + Sync>);

impl StdoutWriter {
    /// Create a new stdout writer wrapper
    pub(in crate::presentation::cli::views) fn new(writer: Box<dyn Write + Send + Sync>) -> Self {
        Self(writer)
    }
    ...
    pub(in crate::presentation::cli::views) fn write_line(&mut self, message: &str) {
    ...
    /// Errors are silently ignored as output operations are best-effort.
    #[allow(dead_code)]
    pub(in crate::presentation::cli::views) fn writeln(&mut self, message: &str) {
...
/// memory layout and performance characteristics as the wrapped type,
/// but provides compile-time type safety.
pub(in crate::presentation::cli::views) struct StderrWriter(Box<dyn Write + Send + Sync>);
...
mod tests {
    use super::*;
    use crate::presentation::cli::views::testing;
