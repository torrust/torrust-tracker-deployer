//! JSON View for Destroy Command
//!
//! This module provides JSON-based rendering for the destroy command.
...
//! # Design
//!
//! The `JsonView` serializes destroy command information to JSON using `serde_json`.
...
/// and AI agents. It serializes the destroy details without any transformations,
...
impl JsonView {
    /// Render destroy details as JSON
    ///
    /// Serializes the destroy details to pretty-printed JSON format.
    /// The JSON structure matches the DTO structure exactly:
    /// - `environment_name`: Name of the environment
    /// - `instance_name`: VM instance name
    /// - `provider`: Infrastructure provider
    /// - `state`: Always "Destroyed" on success
    /// - `instance_ip`: IP address (nullable — `null` if never provisioned)
    /// - `created_at`: ISO 8601 UTC timestamp
    ///
    /// # Arguments
    ///
    /// * `data` - Destroy details to render
    ///
    /// # Returns
    ///
    /// A JSON string containing the serialized destroy details.
    ...
    /// assert!(json.contains("\"state\": \"Destroyed\""));
    /// ```
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn it_should_render_destroy_details_as_valid_json() {
        // Arrange
