...
use super::handler::{
    display_creation_results, execute_create_command, handle_environment_creation,
    load_configuration,
};
...
mod display_creation_results_tests {
    ...
    fn it_should_display_environment_details() {
        ...
        let mut progress = ProgressReporter::new(user_output, 5);
        let environment = execute_create_command(&mut progress, &command_handler, config).unwrap();
...
mod error_tests {
    ...
    use crate::presentation::controllers::create::subcommands::environment::errors::ConfigFormat;
