    ...
    match cli.command {
        Some(Commands::Destroy { environment }) => {
            if let Err(e) = handle_destroy_command(&environment) {
                eprintln!("Error: {e}");
                eprintln!("\nFor detailed troubleshooting, run with --help or see:");
                eprintln!("{}", e.help());
                std::process::exit(1);
            }
        }
...
/// Errors that can occur during the destroy command execution
#[derive(Debug, Error)]
#[allow(dead_code)] // Some variants may be used in future implementations
...
Tip: Environment names must be 1-63 characters, start with letter/digit, contain only letters/digits/hyphens")]
    InvalidEnvironmentName {
        name: String,
        #[source]
        source: torrust_tracker_deployer_lib::domain::environment::name::EnvironmentNameError,
    },
...
Tip: Check logs and try running with --log-output file-and-stderr for more details"
    ...
    DestroyOperationFailed {
        name: String,
        #[source]
        source: torrust_tracker_deployer_lib::application::command_handlers::destroy::DestroyCommandHandlerError,
    },
...
impl DestroyCommandError {
    ...
    ///
    /// ```rust
    /// if let Err(e) = handle_destroy_command("test-env") {
    ///     eprintln!("Error: {e}");
    ///     eprintln!("\nTroubleshooting:\n{}", e.help());
    /// }
    /// ```
...
fn handle_destroy_command(environment_name: &str) -> Result<(), DestroyCommandError> {
    ...
    let env_name = EnvironmentName::new(environment_name.to_string()).map_err(|source| {
        let error = DestroyCommandError::InvalidEnvironmentName {
    ...
    let _destroyed_env = command_handler.execute(&env_name).map_err(|source| {
        let error = DestroyCommandError::DestroyOperationFailed {
            name: environment_name.to_string(),
            source,
        };
        output.error(&error.to_string());
