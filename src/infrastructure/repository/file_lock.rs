...
//! use torrust_tracker_deploy::infrastructure::repository::file_lock::FileLock;
//!
...
impl FileLock {
    ...
    fn lock_file_path(file_path: &Path) -> PathBuf {
        ...
        let current_extension = lock_path
            .extension()
            .and_then(|e| e.to_str())
            .unwrap_or("");
...
mod tests {
    ...
    fn it_should_handle_concurrent_acquisitions_with_threads() {
        ...
        let handle1 = thread::spawn(move || {
            FileLock::acquire(&file_path, Duration::from_secs(2))
        });
