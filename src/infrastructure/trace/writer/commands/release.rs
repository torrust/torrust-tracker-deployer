//! Release command trace writer
//!
//! Generates trace files for release command failures with release-specific
//! context and metadata.

use std::path::{Path, PathBuf};
use std::sync::Arc;

use crate::domain::environment::state::ReleaseFailureContext;
...
/// use std::sync::Arc;
/// use torrust_tracker_deployer_lib::infrastructure::trace::ReleaseTraceWriter;
/// use torrust_tracker_deployer_lib::shared::SystemClock;
///
/// let traces_dir = PathBuf::from("data/my-env/traces");
/// let clock = Arc::new(SystemClock);
/// let writer = ReleaseTraceWriter::new(traces_dir, clock);
/// ```
pub struct ReleaseTraceWriter {
    common: CommonTraceWriter,
}

impl ReleaseTraceWriter {
    /// Create a new release trace writer
    #[must_use]
    ...
    /// Write a release failure trace file
    ///
    /// Generates a trace file with release-specific context and logs the outcome.
    /// Success is logged at INFO level, failures at WARN level.
    ///
    /// # Arguments
    ///
    /// * `ctx` - The release failure context with metadata
    ...
    pub fn write_trace<E: Traceable>(
        ...
        match self.common.write_trace("release", &trace_content) {
            Ok(trace_file_path) => {
                info!(
                    trace_id = %ctx.base.trace_id,
                    trace_file = ?trace_file_path,
                    "Generated trace file for release failure"
                );
                Ok(trace_file_path)
            }
            Err(e) => {
                warn!(
                    trace_id = %ctx.base.trace_id,
                    error = %e,
                    "Failed to generate trace file for release failure"
                );
                Err(e)
            }
        }
    ...
    /// Format a complete release trace
    fn format_trace<E: Traceable>(ctx: &ReleaseFailureContext, error: &E) -> String {
        use std::fmt::Write;

        let mut trace = String::new();

        // Header
        trace.push_str(&TraceSections::header("RELEASE FAILURE TRACE"));
...
mod tests {
    ...
    use crate::domain::environment::state::{
        BaseFailureContext, ReleaseFailureContext, ReleaseStep,
    };
    ...
    fn create_test_writer() -> (ReleaseTraceWriter, TempDir, PathBuf) {
        ...
        let writer = ReleaseTraceWriter::new(traces_dir.clone(), clock);
        (writer, temp_dir, traces_dir)
    }

    /// Create a release failure context with default test values
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// A release failure context with sensible defaults for testing
    fn create_test_context(error_summary: &str) -> ReleaseFailureContext {
        let now = Utc::now();
    ...
    /// Create a release failure context with a specific trace ID
    ///
    ...
    #[test]
    fn it_should_create_release_trace_writer_with_directory() {
        // Arrange
    ...
    #[test]
    fn it_should_write_release_trace_with_correct_naming() {
        // Arrange
        let (writer, _temp_dir, _traces_dir) = create_test_writer();
        let error = create_test_error("release test error");
        let context = create_test_context(&error.to_string());
        ...
        assert!(filename.ends_with("-release.log"));
    }
    ...
    fn it_should_use_timestamp_and_command_as_filename() {
        ...
        // Verify filename format: {timestamp}-release.log
        // Example: 20251003-103045-release.log
        assert!(filename.ends_with("-release.log"));

        // Verify timestamp prefix exists (YYYYmmdd-HHMMSS format)
        let parts: Vec<&str> = filename.split('-').collect();
        assert!(parts.len() >= 3); // At least YYYYmmdd, HHMMSS, release.log
    ...
    #[test]
    fn it_should_include_trace_metadata_in_release_trace() {
        // Arrange
        let (writer, _temp_dir, _traces_dir) = create_test_writer();
        let error = create_test_error("test error");
        ...
        assert!(trace_data.contains("RELEASE FAILURE TRACE"));
        assert!(trace_data.contains(&format!("Trace ID: {trace_id}")));
        assert!(trace_data.contains("Failed Step: Render Docker Compose Templates"));
    ...
    fn it_should_generate_trace_files_with_correct_naming() {
        ...
        // Arrange
        let (writer, _temp_dir, traces_dir) = create_test_writer();
        let error = create_test_error("Simulated release failure");
        let context = create_test_context(&error.to_string());
        ...
        // Verify trace file was created with correct naming: {timestamp}-release.log
        ...
        assert!(
            filename.ends_with("-release.log"),
            "Filename should end with '-release.log', got: {filename}"
        );
    ...
    #[test]
    fn it_should_format_release_step_in_trace() {
        // Arrange
        let (writer, _temp_dir, _traces_dir) = create_test_writer();
        let error = create_test_error("File transfer failed");
        let now = Utc::now();
        ...
        assert!(trace_data.contains("Failed Step: Deploy Compose Files to Remote"));
